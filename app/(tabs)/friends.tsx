...
const Friends = () => {
  ...
  const ViewButton = ({ viewType }: { viewType: ViewType }) => {
    return (
      <TouchableOpacity
        className={cn(
          "p-2 rounded flex-1 justify-center",
          toggleView === viewType && "bg-green-600",
        )}
  ...
  return (
    <SafeAreaView className={"themed-view gap-4 items-center justify-center"}>
      {isAnonymous ? (
        <Text className={"text text-center w-full"}>
          Guest accounts cannot access the friends tab.
        </Text>
      ) : (
        <>
          <View
            className={
              "items-center justify-center p-4 pb-0 border-b-2 border-b-white"
            }
          >
            <Text className={"text text-2xl font-bold"}>Friends Page</Text>
          </View>

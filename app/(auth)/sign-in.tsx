...
const SignIn = () => {
  ...
  const toggleAuthTypeComponent = () => {
    if (authType === "Sign In") {
      return (
        <Text className={"flex-1 text text-center"}>
          Don&#39;t have an account?{" "}
          <Text
            onPress={() => setAuthType("Sign Up")}
  ...
  return (
    <KeyboardAvoidingView className={"gap-6 p-6"}>
      <View className={"gap-6 p-6 rounded-lg bg-gray-700"}>
        ...
        <View className={"gap-4"}>
          ...
          <CustomTextInput
            ...
            value={form.password}
            secureText={!showFormPassword}
            icon={
              <Ionicons
                name={!showFormPassword ? "eye" : "eye-off"}
                size={24}
                color={"white"}
        ...
        >
          <Text className={"text text-xl text-center"}>{authType}</Text>
        </TouchableOpacity>
        <View className={"flex flex-row"}>{toggleAuthTypeComponent()}</View>
      </View>
      <Text className={"text text-center"}>or</Text>
      <TouchableOpacity
        onPress={onPressSignInAnonymous}
        className={"bg-blue-500 py-2 rounded-lg"}
      >
        <Text className={"text text-xl text-center"}>Sign in as Guest</Text>
      </TouchableOpacity>
    </KeyboardAvoidingView>
  );
};

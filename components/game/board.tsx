...
const Board = () => {
  const { game, canCounter, player, playerError, showAllComments } =
    useGameStore();
  ...
  const playedCardsMapRef = useRef<
    Map<PlayedCards, React.RefObject<View | null>>
  >(new Map()); // Map of View references for playedCard pairs with JUST ATK CARD
  ...
  const [hoveredPlayedCards, setHoveredPlayedCards] =
    useState<PlayedCards | null>(null);
  const playerCardBoardRefs: PlayerCardBoardRefs = {
    hoveredOverBoardRef,
    hoveredPlayedCardsRef,
  };

  // Update the playedCardsMapRef each time playedCards updates
  useEffect(() => {
    game &&
      game.playedCards.map((item, index) => {
        if (!item.defendingCard) {
          const itemRef = playedCardsMapRef.current.get(item);
          playedCardsMapRef.current.set(
            item,
            itemRef ? itemRef : React.createRef<View>(),
          );
        } else {
  ...
  const handleDragMove = (dx: number, dy: number) => {
    ...
    ghostCardRef.current.measureInWindow((x, y, width, height) => {
      const ghostCardLayout: LayoutRectangle = {
        x,
        y,
        width: width,
        height: height,
  ...
  return (
    <View className={"flex-1"}>
      ...
      <View className={"flex-1 flex-row items-center justify-between"}>
        ...
        <View
          className={cn(
            "p-2 pb-8 rounded relative",
            (canCounter || hoveredOverBoard) && "bg-gray-700",
          )}
        ...
        >
          <PlayedCardsList
            ...
            getPlayedCardsRef={getPlayedCardsRef}
            hoveredPlayedCards={hoveredPlayedCards}
          />
          {canCounter && (
            <Text
              className={
                "text text-2xl text-center absolute bottom-4 left-1/2 transform -translate-x-1/2"
              }
            >
              Counter!
            </Text>
          ...
          {playerError && (
            <Text
              className={
                "text text-sm text-center absolute bottom-4 left-1/2 transform -translate-x-1/2"
              }
            >
              {playerError}
            </Text>
      ...
      <UserPlayerRow
        ...
        cardDragImpl={ghostCardDragImpl}
      />
    </View>
  );
};

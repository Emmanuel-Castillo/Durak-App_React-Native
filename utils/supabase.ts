...
export const getSentRequests = async (sender: User) => {
  try {
    const { data: friendRequests, error: friendRequestErrors } = await supabase
...
export const approveFriendRequest = async (request: FriendRequest) => {
  try {
    ...
    const { data: insertFriendData, error: insertFriendError } = await supabase
      .from("friends")
      .insert({
...
export const deleteFriendRequest = async (request: FriendRequest) => {
  try {
    const { data: deleteRequestData, error: deleteRequestError } =
...
export const updatePushToken = async (user: User, token: string) => {
  try {
    await supabase
      ...
      .upsert({ user_id: user.id, expo_push_token: token });
  } catch (e: any) {
    console.log(e);
    Alert.alert(e.toString());
  }
...
export const updateGameWins = async (user: User) => {
  try {
    const { data, error } = await supabase
      .from("profiles")
      .update({ num_wins: user.num_wins + 1 })
      .eq("id", user.id);
    if (error) throw error;
    return;
  } catch (e: any) {
